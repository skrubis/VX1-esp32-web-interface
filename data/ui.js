...
var ui = {
  ...
  setNodeIdAndSpeed: function() {
    ...
    var map = {};
		...
		map.gain = parseFloat(document.getElementById('cangain').value);
		map.offset = parseInt(document.getElementById('canoffset').value);
		// send new CAN mapping to inverter
		inverter.addCanMapping(ui.populateExistingCanMappingTable, map);
		// hide form
    ...
    for (var i = 0; i < values.length; i++) {
      ...
      if (typeof param.id !== 'undefined') {
        ...
        canNameCell.innerHTML = param.name;
        // tx/rx
        ...
        var canOffsetCell = tr.insertCell(-1);
            canOffsetCell.innerHTML = mapping.position;
            // canlength
        var canLengthCell = tr.insertCell(-1);
            canLengthCell.innerHTML = mapping.length;
        // cangain
        var canGainCell = tr.insertCell(-1);
            canGainCell.innerHTML = mapping.gain;
        //can offset
        var canOffsetCell = tr.insertCell(-1);
            canOffsetCell.innerHTML = mapping.offset;

        // delete button
        var canDeleteCell = tr.insertCell(-1);
        var cmd = "inverter.canDelete(ui.populateExistingCanMappingTable, " + mapping.index + "," + mapping.subindex + ");";
